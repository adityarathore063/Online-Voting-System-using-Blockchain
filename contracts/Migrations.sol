...
contract Vote {
    ...
    function candidateVerification(string calldata _party)
        internal
        view
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < arr.length; i++) {
            if (keccak256(abi.encodePacked(arr[i].party))== keccak256(abi.encodePacked(_party))) {
                return false;
            }
        }
        return true;
    ...
    function voteTime(uint256 _startTime, uint256 _endTime) external {
        require(startVoting == false, "Voting already Started");
        require(
            msg.sender == electionCommission,
            "You are not from Election Commision"
        );
